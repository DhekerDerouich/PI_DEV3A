...
import tn.esprit.farmvision.Finance.dao.DepenseDAO;
import tn.esprit.farmvision.Finance.dao.RevenuDAO;
import tn.esprit.farmvision.Finance.model.Depense;
import tn.esprit.farmvision.Finance.model.Revenu;
import tn.esprit.farmvision.Finance.service.PDFEndpointClient;
import tn.esprit.farmvision.Finance.service.aiFinance;
...
public class KPIController {
    ...
    public void initialize() {
        ...
        // Agrandir le PieChart par défaut
        Platform.runLater(() -> {
            pieChart.setPrefSize(650, 450);
    ...
    private void loadPieChart(Map<String, Double> categories) {
        ...
        // Trier les catégories par montant décroissant pour une meilleure lisibilité
        List<Map.Entry<String, Double>> sortedCategories = new ArrayList<>(categories.entrySet());
        sortedCategories.sort((a, b) -> Double.compare(b.getValue(), a.getValue()));

        double total = sortedCategories.stream().mapToDouble(Map.Entry::getValue).sum();
        ...
        for (Map.Entry<String, Double> entry : sortedCategories) {
            ...
            pieChart.getData().add(new PieChart.Data(
                    label + " (" + String.format("%.0f", entry.getValue()) + " TND)",
                    entry.getValue()
            ));
        }

        Platform.runLater(() -> {
            // Agrandir le PieChart
            pieChart.setPrefSize(700, 500);
            pieChart.setLabelsVisible(true);
            pieChart.setLabelLineLength(20);
            pieChart.setStartAngle(90); // Commencer à partir du haut
            ...
            for (PieChart.Data data : pieChart.getData()) {
                if (data.getNode() != null) {
                    ...
                    data.getNode().setOnMouseExited(e -> {
                        ...
                        data.getNode().setStyle("-fx-pie-color: " + getColorForIndex(currentIndex) + ";");
                    });
    ...
    private String getColorForIndex(int index) {
        // Palette de couleurs plus variée
        String[] colors = {
                "#ef4444", // Rouge
                "#f97316", // Orange
                "#eab308", // Jaune
