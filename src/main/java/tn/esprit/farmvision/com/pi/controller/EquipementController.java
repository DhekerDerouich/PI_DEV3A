...
import tn.esprit.farmvision.com.pi.service.QRCodeService;
import tn.esprit.farmvision.com.pi.service.external.CO2SignalService;
...
public class EquipementController {
    ...
    public void initialize() {
        ...
        if (!SessionManager.getInstance().isResponsable() && !SessionManager.getInstance().isAdmin()) {
            showError("Accès refusé", "Seuls les Responsables d'Exploitation peuvent accéder à ce module");
            // Redirection vers dashboard approprié
            redirectToDashboard();
            return;
        }

        System.out.println("✅ Accès autorisé pour: " + SessionManager.getInstance().getCurrentUser().getClass().getSimpleName());
    ...
    private void planifierMaintenance(Equipement equipement) {
        try {
            ...
            if (result.isPresent() && result.get() == planifierButton) {
                ...
                tn.esprit.farmvision.com.pi.service.MaintenanceService maintenanceService =
                        new tn.esprit.farmvision.com.pi.service.MaintenanceService();                maintenanceService.planifierMaintenance(
                        equipement.getId(),
                        typeBox.getValue(),
